...
class Video_Finder(object):
    ...
    def collect_episodes(self):   # Scrapes the list of episodes frook om the video host indexing site
    ...
    def choose_source(self):   # Reorders the list of sources by reliability and helps the user choose one
        ...
        for (url, host, vote) in self.sources:
            ...
            if soup != "Broken Link":
                ...
                if any(words_found):
                    print("Yeah... no. The one from %s is crap. I found these words: %s. Lemme check another..."
                          % (host, ", ".join(words_found)))
                    if host in self.hosttable:
